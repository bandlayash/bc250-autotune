...
def _row_voltage(row: dict[str, Any]) -> str | None:
    """The configured GPU voltage for a run.

    Prefers the recorded field; falls back to scraping a label like
    ``oc-1600-875mv`` for rows written before the field existed. Returns None
    rather than guessing, so the cell renders as an em dash.
    """
    import re as _re

    volts = row.get("gpu_voltage_mv") or {}
    low, high = volts.get("min"), volts.get("max")
    if low is not None and high is not None:
        return f"{low} mV" if low == high else f"{low}–{high} mV"

    match = _re.search(r"(\d+)\s*mv", str(row.get("config_label", "")), _re.I)
    return f"{match.group(1)} mV" if match else None
...
def build_section(rows: list[dict[str, Any]]) -> str:
    ...
    if baseline and tuned:
        parts += [
            ...
            f"| GPU voltage | {_fmt(_row_voltage(baseline))} | {_fmt(_row_voltage(tuned))} | |",
            f"| Started at | {_fmt(baseline.get('start_temp_c'), ' °C', 1)} | "
            f"{_fmt(tuned.get('start_temp_c'), ' °C', 1)} | |",
    ...
    for row in completed[-15:]:
        parts.append(
            ...
            f"{_row_config(row)} | {_fmt(row.get('start_temp_c'), ' °C', 1)} | "
    ...
    # Emitted with every table, not written by hand, so it cannot drift from
    # the numbers or be forgotten on a regenerate. Without it a reader would
    # reasonably assume "stock" meant a stock BC-250, which it does not.
    parts += [
        '> **What "stock" means here.** Only the *GPU governor configuration* is '
        "stock. Both runs were taken on a machine with the **40 CU unlock and "
        "the 8c/16t CPU core unlock already active** — 40 of 40 compute units "
        "(20 WGPs) applied by a BC-250-patched kernel via "
        "`amdgpu.bc250_cc_write_mode=3`, and 16 threads online against the "
        "factory 6c/12t.",
        "",
        "> A stock-topology BC-250 (24 CU, 6c/12t) will score lower on **both** "
        "sides. These numbers isolate the effect of the governor configuration "
        "with the unlocks held constant; the unlocks themselves are a separate "
        "and almost certainly larger lever.",
        "",
    ]

    parts.append(
        "<sub>Generated by `benchmarks/scripts/generate_readme_benchmarks.py`. "
        "FurMark runs start from a cooled GPU (60 °C gate) so results are "
        "comparable; every row records the temperature it actually started "
        "at.</sub>"
    )
    return "\n".join(parts)
